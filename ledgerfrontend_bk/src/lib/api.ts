...
function initTokens(): void {
  ...
  try {
    const access =
      sessionStorage.getItem(TOKEN_KEY_ACCESS) || localStorage.getItem(TOKEN_KEY_ACCESS);
    const refresh =
      sessionStorage.getItem(TOKEN_KEY_REFRESH) || localStorage.getItem(TOKEN_KEY_REFRESH);
